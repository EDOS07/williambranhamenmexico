...
export default function IglesiasDashboard() {
    const [loading, setLoading] = useState(true);
    const [iglesias, setIglesias] = useState<Iglesia[]>([]);
    const [formDataIg, setFormDataIg] = useState<Iglesia>({
        id: null,
        tabernaculo: "",
        pastor: "",
        correo: "",
    ...
    // Manejar cambios en el formulario Iglesia
    const handleChangeIg = (e: React.ChangeEvent<HTMLInputElement>) => {
        setFormDataIg({
            ...formDataIg,
            [e.target.name]:
                e.target.name === "horarios" ? JSON.parse(e.target.value) : e.target.value,
        });
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const url = formDataIg.id
            ? `/api/admin/iglesias?id=${formDataIg.id}`
            : "/api/admin/iglesias";
        ...
        try {
            const response = await fetch(url, {
                ...
                body: JSON.stringify({
                    ...formDataIg,
                    horarios: Array.isArray(formDataIg.horarios)
                        ? formDataIg.horarios
                        : [],
                }),
            });
    ...
    return (
        <>
                        ...
                        {iglesias.map((iglesia: Iglesia) => (
                            <tr key={iglesia.id} className="border">
                                ...
                                <td className="border p-2">
                                    {iglesia.horarios.map((horario, index) => (
                                        <div key={index}>
            ...
            <form onSubmit={handleSubmit} className="mb-10 p-4 text-black">
                <h2 className="text-2xl mb-4">
                    {formDataIg.id ? "Editar Iglesia" : "Crear Iglesia"}
                </h2>
                <input
                    type="text"
                    name="tabernaculo"
                    ...
                    className="p-2 mb-2 w-full font-bold bg-white"
                    required
                />
                <input
                    type="text"
